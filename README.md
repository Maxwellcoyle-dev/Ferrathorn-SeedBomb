...
```bash
SeedBomb$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
SeedBomb$ sam local start-api
SeedBomb$ curl http://localhost:3000/
```
...
```bash
SeedBomb$ sam logs -n HelloWorldFunction --stack-name SeedBomb --tail
```
...
```bash
sam delete --stack-name SeedBomb
```
